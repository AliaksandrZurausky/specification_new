...
<head>
  ...
  <title>Создать спецификацию</title>
  <style>
    :root {
      --c-text: #1f2a37;
      --c-sub: #525c69;
...
<div class="spec-page">
  <div class="spec-card">
    <h2 class="spec-title">Создание спецификации</h2>

    <div class="ui-layout-form" id="spec-create-form">
      <!-- SPEC (UF) -->
      ...
      <div class="ui-layout-form-row">
        ...
        <div class="ui-layout-form-content">
          <div class="ui-ctl ui-ctl-textbox ui-ctl-w100">
            <input class="ui-ctl-element" type="text" id="specname" name="NAME" value="">
          </div>
        </div>
      </div>
...
BX.ready(function () {
  ...
  const RAW_IBLOCK_ID = 14; // Товарный каталог
  const SPEC_FIELD = 'UF_CRM_27_1767679276';

  const specWrap = document.getElementById('spec-field-wrap');

  function renderElementSelector(containerId, fieldName, iblockId) {
  ...
  // SPEC selector
  renderElementSelector('spec-selector', SPEC_FIELD, RAW_IBLOCK_ID);

  function getSpecHidden() {
    return document.querySelector(`[name="${SPEC_FIELD}"]`);
  }

  function getSpecDefault() {
    return document.getElementById(`${SPEC_FIELD}_default`);
  }
  ...
  function syncName(force = false) {
    ...
    const specChanged = currentSpecValue !== lastAppliedSpecValue;
    if (force || specChanged) {
      nameInput.value = title;
      lastAutoName = title;
      nameTouched = false;
      lastAppliedSpecValue = currentSpecValue;
    }
  }
  ...
  // IMPORTANT: fields inside SidePanel may be rendered/replaced after BX.ready.
  // Do not rely on cached DOM references for UF hidden inputs.

  // Listen to *_default if it exists; also re-try attaching once later.
  function bindDefaultChange() {
    const specDefault = getSpecDefault();
    if (specDefault && !specDefault.__specBound) {
      specDefault.__specBound = true;
      specDefault.addEventListener('change', function () { syncName(true); });
    }
  }
  ...
  // React to any selector DOM changes
  ...
  function setRowInputsEnabled(rowEl, enabled) {
  ...
  function watchRawId(rowEl, index) {
    ...
    function handleChange(rawId) {
      if (rawId <= 0) {
        setRowInputsEnabled(rowEl, false);
        return;
      }

      setRowInputsEnabled(rowEl, true);
  ...
  function renderRawRow(index) {
      ...
      <div class="raw-row-grid">
        ...
        <div>
          ...
          <div class="ui-ctl ui-ctl-after-icon ui-ctl-dropdown ui-ctl-w100">
            ...
            <select class="ui-ctl-element" name="RAW[${index}][UNIT]" disabled>
        ...
        <div>
          ...
          <div class="ui-ctl ui-ctl-textbox ui-ctl-w100">
            <input class="ui-ctl-element" type="number" step="any" min="0" inputmode="decimal" name="RAW[${index}][QTY]" value="" disabled>
          </div>
        </div>
    ...
    // RAW selector (same type as SPEC)
    const selector = renderElementSelector(rawSelectorId, `RAW[${index}][RAWID]`, RAW_IBLOCK_ID);
    if (selector) {
      rowSelectors.set(index, selector);
    }

    // Start watching after render (FieldSelector may inject hidden input a bit later)
    rawWatcher = watchRawId(row, index);
  }
  ...
  function closeSliderAndReload() {
    ...
    // Важно: получать ссылку на текущий слайдер нужно ДО закрытия.
  ...
  saveBtn.addEventListener('click', function () {
    ...
    const rows = Array.from(document.querySelectorAll('#raw-list .raw-row'))
      .map(function (row) {
        ...
        return { rawId: rawId, qty: qty, unit: unit, title: rowTitle };
      })
      .filter(function (r) {
        return r.rawId > 0 && r.qty > 0 && r.unit !== '';
      });
    ...
    BX.ajax({
      url: '/local/specifications/create/save_specification.php',
      method: 'POST',
      dataType: 'json',
      data: {
        sessid: BX.bitrix_sessid(),
      ...
      onsuccess: function (res) {
        ...
        showTopNotification('Спецификация сохранена');
